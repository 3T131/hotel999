       ...
       <table class="table table-condensed table-bordered table-striped" id="tableid">
	      ...
	      <tbody id="tbody">
	        <c:forEach items="${pager.datas}" var="item">
		        <tr >
		          <td><input type="radio" name="id" value="${item.stayRegisterId}"></td>
		          <td>${item.roomNumber}</td>
				  <td>${item.receiveTarget.principal}</td>
		          <td>${item.passengerList[0].name}</td>
		          <td>${item.changingRoomNumber}</td>
		          <td><fmt:formatDate value="${item.changRoomTime}"/></td>
		          <td><fmt:formatDate value="${item.registerTime}"/></td>
				  <td><fmt:formatDate value="${item.payTime}"/></td>
		          <td>${item.sumConst}</td>
		          <td hidden>${item.stayRegisterId}</td>
		        </tr>
	        </c:forEach>
	      </tbody>
	    </table>
 ...
 <script type="text/javascript">
   ...
   function particulars(){
     ...
     $('input[name="id"]:checked').each(function(){             // 遍历获取input被checked的
  	...
  	if(id!=""){
  	   parent.document.getElementById('Mainid').src='${ctx}/FinancialStatistics/toinformation.do?stayRegisterId='
  	   +id+'&dateMin='+datemin+'&dateMax='+datemax;
  	}else{
  	   alert("请选择一个房间再按 详情");
  	}
   }
